...
var fs = require('fs');
var files = fs.readdirSync('src/img/decor/');
...
files.forEach((file) => {
	...
	arrFiles.push(file.replace(/.jpg/, ''));
});

const pathTo = 'src/img/decor/';
	...
	return new Promise((resolve) => {
		function foo22() {
			...
			for (let i = 0; i < arr.length; i++) {
				let path = 'src/img/decor/' + arr[i] + fileExtension;
...
module.exports = function () {
	async function jimpFoo() {
		const result = await fooDecor();
		return result;
	}

	$.gulp.task('jimpDecor', function () {
		return jimpFoo();
	});
};
